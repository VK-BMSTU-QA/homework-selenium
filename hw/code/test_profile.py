...
class TestProfileContacts(BaseCase):
    ...
    def test_change_password(self, request: FixtureRequest):
        ...
        password_input = self.profile_page.find(
            self.profile_page.locators.PASSWORD_INPUT, 10
        )
        self.profile_page.send_keys(password_input, self.NORMAL_PASS)

        password_copy_input = self.profile_page.find(
            self.profile_page.locators.PASSWORD_COPY_INPUT, 10
        )
        self.profile_page.send_keys(password_copy_input, self.NORMAL_PASS)

        self.profile_page.click(self.profile_page.locators.SAVE_BUTTON, 15)

        notification = self.profile_page.wait_visability_of_elem(
            self.profile_page.locators.SAVE_NOTIFICATION
        )

        assert str(notification.text) == "Информация обновлена!"

        self.profile_page.move_on_element(
            self.profile_page.locators.PROFILE_PIC, 3
        )
    ...
    def test_change_username(self):
        ...
        input_field = self.profile_page.find(
            self.profile_page.locators.LOGIN_INPUT, 15
        )
        ...
        self.base_page.click(self.profile_page.locators.SAVE_BUTTON, 15)

        self.base_page.wait_visability_of_elem(
            self.profile_page.locators.SAVE_NOTIFICATION, 15
        )

        updated_input_field = self.base_page.find(
            self.profile_page.locators.BUTTON_PROFILE_1, 15
        )
        updated_input_field_text = updated_input_field.text

        self.profile_page.driver.refresh()
        ...
        another_updated_input_field = self.base_page.find(
            self.profile_page.locators.BUTTON_PROFILE_1, 15
        )

        assert updated_input_field_text == self.NEW_USERNAME
    ...
    def test_wrong_inputs(self):
        ...
        login_input = self.reg_page.find(
            self.profile_page.locators.LOGIN_INPUT, 10
        )
        self.reg_page.send_keys(login_input, "")

        notification = self.profile_page.wait_visability_of_elem(
            self.profile_page.locators.PASSWORD_NOTIFICATION
        )
        ...
        login_input = self.profile_page.find(
            self.profile_page.locators.LOGIN_INPUT, 10
        )
        ...
        notification = self.profile_page.wait_visability_of_elem(
            self.profile_page.locators.PASSWORD_COPY_NOTIFICATION
        )
